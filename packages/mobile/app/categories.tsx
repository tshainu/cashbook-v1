...
import { ArrowLeft, Tag, Trash, PencilSimple, Check, X, Plus } from "phosphor-react-native";
    ...
    <View style={sw.wrapper}>
      ...
      <Animated.View style={[sw.row, { transform: [{ translateX: tx }] }]} {...pan.panHandlers}>
        <View style={sw.iconBox}>
          <Tag size={16} color={RED} weight="fill" />
        </View>
        <View style={sw.info}>
...
function EditCard({ item, onDone, onCancel }: {
  ...
  async function save() {
    ...
    try {
      const res = await api.items[":id"].$put({
        param: { id: item.id },
        json: { name: name.trim(), defaultPrice: parseFloat(price) || 0, type: "expense" },
      });
  ...
  return (
    <View style={ec.card}>
      ...
      <TextInput style={ec.input} value={name} onChangeText={setName} placeholder="Category name" placeholderTextColor="#C0A8A5" />
      <TextInput style={ec.input} value={price} onChangeText={setPrice} placeholder="Default price" keyboardType="decimal-pad" placeholderTextColor="#C0A8A5" />
      <View style={ec.row}>
        <TouchableOpacity style={[ec.btn, { backgroundColor: RED }]} onPress={save} disabled={saving}>
...
export default function CategoriesScreen() {
  ...
  const { data: categories = [], isLoading } = useQuery<Category[]>({
    ...
    queryFn: async () => {
      const res = await api.items.$get({ query: { shopId: String(shopId), type: "expense" } });
      if (!res.ok) throw new Error();
  ...
  async function handleAdd() {
    ...
    try {
      const res = await api.items.$post({
        json: { shopId, name: newName.trim(), defaultPrice: parseFloat(newPrice) || 0, type: "expense" },
      });
      if (!res.ok) throw new Error();
  ...
  return (
    <SafeAreaView style={s.safe} edges={["top"]}>
      <View style={s.header}>
        ...
        <View>
          <Text style={s.headerTitle}>Expense Categories</Text>
        ...
        <FlatList
            ...
            <>
              ...
              <View style={s.section}>
                ...
                <View style={s.addCard}>
                  <TextInput style={s.input} placeholder="Category name" placeholderTextColor="#C0A8A5" value={newName} onChangeText={setNewName} />
                  <TextInput style={s.input} placeholder="Default price (optional)" placeholderTextColor="#C0A8A5" keyboardType="decimal-pad" value={newPrice} onChangeText={setNewPrice} />
          ...
          ListEmptyComponent={
            isLoading
              ...
              : <View style={s.emptyBox}><Tag size={36} color="#DDD" weight="duotone" /><Text style={s.emptyText}>No categories yet</Text></View>
          }
          renderItem={({ item }) => (
...
const s = StyleSheet.create({
  ...
  input: {
    borderWidth: 1.5, borderColor: "#F0D8D5", borderRadius: 10,
    paddingHorizontal: 12, paddingVertical: 11,
    fontSize: 15, color: "#2E1A18", marginBottom: 10, backgroundColor: "#FDF7F6",
  },
  addBtn: {
    backgroundColor: RED, borderRadius: 10, paddingVertical: 13,
...
const sw = StyleSheet.create({
  ...
  info: { flex: 1 },
  name: { fontSize: 15, fontWeight: "600", color: "#2E1A18" },
...
const ec = StyleSheet.create({
  ...
  title: { fontSize: 13, fontWeight: "700", color: "#F0A500", marginBottom: 12, textTransform: "uppercase", letterSpacing: 0.5 },
  input: {
    borderWidth: 1.5, borderColor: "#E4E8EF", borderRadius: 10,
    paddingHorizontal: 12, paddingVertical: 10,
    fontSize: 14, color: "#2E1A18", marginBottom: 8, backgroundColor: "#FAFBFC",
  },
